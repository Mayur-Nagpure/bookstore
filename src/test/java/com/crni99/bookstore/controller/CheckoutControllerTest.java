...
class CheckoutControllerTest {
	...
	void shouldReturnCheckout() {
		...
		when(shoppingCartService.getshippingCosts()).thenReturn(Double.valueOf("10"));
	...
	void shouldPlaceOrder() {
		...
		when(shoppingCartService.getCart()).thenReturn(cart);
		doNothing().when(billingService).createOrder(customer, cart, "CASH");
		...
		String result = checkoutController.placeOrder(customer, bindingResult, "CASH",redirect);

		assertThat(expectedView).isEqualTo(result);
		verify(shoppingCartService, times(1)).getCart();
		verify(billingService, times(1)).createOrder(customer, cart, anyString());
